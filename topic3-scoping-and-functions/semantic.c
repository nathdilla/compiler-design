...
// Update create_operand function
char *create_operand(ASTNode *node)
{
    ...
    case NodeType_SimpleExpr:
        if (is_bool_value(node->simpleExpr.number))
        {
            return strdup(node->simpleExpr.number);
        }
        else if (is_float_value(node->simpleExpr.number))
        {
            return strdup(node->simpleExpr.number);
        }
        else
